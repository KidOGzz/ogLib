...
public final class ArrowTracker implements Listener {
    ...
    public void onEntityShootBow(EntityShootBowEvent event) {
        if (event.getEntity() instanceof Player) {
            event.getProjectile().setMetadata("ShotFromDistance", new FixedMetadataValue(ogLib.getInstance(), event.getProjectile().getLocation()));
        }
    }
    ...
    public static class ArrowDamageByMob extends MobDamage {
        ...
        public String getDeathMessage(UUID getFor) {
            return ArrowDamageByMob.wrapName(this.getDamaged(), getFor) + ChatColor.YELLOW + " was shot by a " + ChatColor.RED + EntityUtils.getName(this.getMobType()) + ChatColor.YELLOW + ".";
        }
    }
    ...
    public static class ArrowDamageByPlayer extends PlayerDamage {
        ...
        @Override
        public String getDeathMessage(UUID getFor) {
            return ArrowDamageByPlayer.wrapName(this.getDamaged(), getFor) + ChatColor.YELLOW + " was shot by " + ArrowDamageByPlayer.wrapName(this.getDamager(), getFor) + ChatColor.YELLOW + " from " + ChatColor.BLUE + (int)this.distance + " blocks" + ChatColor.YELLOW + ".";
        }
    }
