...
public final class FrozenScoreboard {
    ...
    public FrozenScoreboard(UUID player) {
        this.player = player;
        Scoreboard board = ogLib.getInstance().getServer().getScoreboardManager().getNewScoreboard();
    ...
    private String[] separate(String line, Collection<String> usedBunkerscores) {
        ...
        switch (this.separateScores.size()) {
            ...
            default: {
                ogLib.getInstance().getLogger().warning("Failed to separate scoreboard line. Input: " + line);
            }
        }
        if (usedBunkerscores.contains(score)) {
            if (score.length() <= 14) {
                ...
                if (usedBunkerscores.contains(score)) {
                    ogLib.getInstance().getLogger().warning("Failed to find alternate color code for: " + score);
                }
            } else {
                ogLib.getInstance().getLogger().warning("Found a scoreboard base collision to shift: " + score);
            }
        }
        if (prefix.length() > 16) {
