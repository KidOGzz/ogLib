...
public class FrozenTabHandler {
    ...
    public static void init() {
        if (ogLib.getInstance().getConfig().getBoolean("disableTab", false)) {
            return;
        }
        ...
        new TabThread().start();
        ogLib.getInstance().getServer().getPluginManager().registerEvents(new TabListener(), ogLib.getInstance());
    }
    ...
    private static PropertyMap fetchSkin() {
        String propertyMap = ogLib.getInstance().runBackboneRedisCommand(redis -> redis.get("propertyMap"));
