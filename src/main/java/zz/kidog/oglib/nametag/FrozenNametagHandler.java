...
import zz.kidog.oglib.packet.ScoreboardTeamPacketMod;
...
public class FrozenNametagHandler {
    ...
    public static void init() {
        if (ogLib.getInstance().getConfig().getBoolean("disableNametags", false)) {
            return;
        }

        Preconditions.checkState(!FrozenNametagHandler.initiated);
        FrozenNametagHandler.initiated = true;
        FrozenNametagHandler.nametagRestrictionEnabled = ogLib.getInstance().getConfig().getBoolean("NametagPacketRestriction.Enabled", false);
        FrozenNametagHandler.nametagRestrictBypass = ogLib.getInstance().getConfig().getString("NametagPacketRestriction.BypassPrefix").replace("&", "§");
        new NametagThread().start();
        ogLib.getInstance().getServer().getPluginManager().registerEvents(new NametagListener(), ogLib.getInstance());
    ...
    public static void reloadOthersFor(Player refreshFor) {
        for (Player toRefresh : ogLib.getInstance().getServer().getOnlinePlayers()) {
    ...
    protected static void applyUpdate(NametagUpdate nametagUpdate) {
        Player toRefreshPlayer = ogLib.getInstance().getServer().getPlayer(nametagUpdate.getToRefresh()); // does this work with disguises lol idk LOL why dont we just use uuids
        if (toRefreshPlayer == null) {
            return;
        }
        if (nametagUpdate.getRefreshFor() == null) {
            for (Player refreshFor : ogLib.getInstance().getServer().getOnlinePlayers()) {
                reloadPlayerInternal(toRefreshPlayer, refreshFor);
            }
        }
        else {
            Player refreshForPlayer = ogLib.getInstance().getServer().getPlayer(nametagUpdate.getRefreshFor());
    ...
    protected static void reloadPlayerInternal(Player toRefresh, Player refreshFor) {
        if (!refreshFor.hasMetadata("ogLibNametag-LoggedIn")) {
            return;
        }
