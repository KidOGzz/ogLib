...
public class CommandNode {
    ...
    public boolean invoke(CommandSender sender, Arguments arguments) throws CommandException {
        ...
        try {
            ...
            stopwatch.stop();
            int executionThreshold = ogLib.getInstance().getConfig().getInt("Command.TimeThreshold", 10);
            if (!this.async && this.logToConsole && stopwatch.elapsedMillis() >= executionThreshold) {
                ogLib.getInstance().getLogger().warning("Command '/" + this.getFullLabel() + "' took " + stopwatch.elapsedMillis() + "ms!");
            }
            return true;
        }
