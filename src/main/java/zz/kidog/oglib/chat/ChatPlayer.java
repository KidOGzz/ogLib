...
public class ChatPlayer {
    ...
    public void setActiveType(ChatPopulator chatPopulator) {
        ...
        if(isSelected(chatPopulator)) {
            player.sendMessage(ogLib.isBridge() ? ChatColor.YELLOW + "You are already talking in " + ChatColor.RED + chatPopulator.getName() + ChatColor.YELLOW + " chat." : ChatColor.GRAY + "You are already talking in the " + ChatColor.AQUA + chatPopulator.getName() + ChatColor.GRAY + " chat channel.");
            return;
        }
        setSelectedPopulator(chatPopulator);
        player.sendMessage(ogLib.isBridge() ? ChatColor.YELLOW + "You are now talking in " + ChatColor.RED + chatPopulator.getName() + ChatColor.YELLOW + " chat." : ChatColor.GRAY + "You are now talking in the " + ChatColor.AQUA + chatPopulator.getName() + ChatColor.GRAY + " chat channel.");
    }
