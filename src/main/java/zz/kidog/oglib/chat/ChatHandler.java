...
import zz.kidog.oglib.util.CancellationDetector;
...
public class ChatHandler implements Listener {
    ...
    public static void init() {
        ...
        initiated = true;
        Bukkit.getPluginManager().registerEvents(new ChatHandler(), ogLib.getInstance());
        ChatHandler.getDetector().addListener((plugin, event) -> {
            if(plugin != ogLib.getInstance()) ChatHandler.getChatPlayer(event.getPlayer().getUniqueId()).setChatCancelled(true);
        });
    }
    ...
    public void onJoin(PlayerJoinEvent event) {
        ...
        Bukkit.getScheduler().runTaskLater(ogLib.getInstance(), () -> {
