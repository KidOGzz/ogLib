...
public class CombatLoggerListener implements Listener {
    ...
    public void onEntityDeath(EntityDeathEvent event) {
        CombatLogger logger;
        if (event.getEntity().hasMetadata("ogLib-CombatLogger") && (logger = FrozenCombatLoggerHandler.getCombatLoggerMap().get(event.getEntity().getUniqueId())) != null) {
            Player target;
            EntityPlayer entity;
    ...
    public void onEntityInteract(PlayerInteractEntityEvent event) {
        if (event.getRightClicked().hasMetadata("ogLib-CombatLogger")) {
    ...
    public void onChunkUnload(ChunkUnloadEvent event) {
        for (Entity entity : event.getChunk().getEntities()) {
            if (!entity.hasMetadata("ogLib-CombatLogger") || entity.isDead()) continue;
            event.setCancelled(true);
        }
    }
    ...
    public void onEntityPortal(EntityPortalEvent event) {
        if (event.getEntity().hasMetadata("ogLib-CombatLogger")) {
            event.setCancelled(true);
        }
    }
    ...
    public void onEntityPressurePlate(EntityInteractEvent event) {
        ...
        if (pressurePlate && event.getEntity().hasMetadata("ogLib-CombatLogger")) {
            event.setCancelled(true);
        }
    }
